    ...
    for (PerimeterGeneratorArachneExtrusion& pg_extrusion : pg_extrusions) {
        ...
        if (!paths.empty()) {
            ...
            if (extrusion->is_closed) {
                ...
                // Conflict resolved as "ours" — dennisklappe's variant requires a
                // wall_direction PrintConfig key we don't have. Wave overhangs do
                // not strictly need the per-loop inset_idx since line 542 below
                // already sets it from the local inset_idx variable.
