...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config)
{
    ...
    // LeanSpectrum: Wave-overhangs conditional visibility.
    // Master toggle off → hide every wave_overhang_* tunable (only master stays).
    // Skipped dennisklappe's enable_wrapping_detection toggle — depends on a
    // DevPrinterConfigUtil class we don't have.
    ...
    // Corner-reinforcement sub-options. Master toggle reveals the three tunables.
    ...
    // Hilbert floor sub-options. Only meaningful when the master Hilbert toggle
    // is on AND wave overhangs themselves are enabled.
