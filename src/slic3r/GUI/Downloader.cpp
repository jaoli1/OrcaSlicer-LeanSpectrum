...
{
    // Upstream-snap 4b5fc8c3cd: use libcurl's URL parser so any query
    // string ("?foo=bar") and percent-encoded characters are stripped
    // before we hand the leaf to the local filesystem. The old
    // find_last_of("/") strategy left ?query in the filename, which
    // broke download-to-local for any model URL with parameters.
    std::string result;
    CURLU* h = curl_url();
    if (h) {
    ...
    // Fallback: strip query string manually if libcurl rejected the URL.
    if (result.empty()) {
