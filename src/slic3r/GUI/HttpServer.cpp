...
std::shared_ptr<HttpServer::Response> HttpServer::web_server_handle_request(const std::string& url)
{
    ...
    // The /wcp_download/ route carries paths that were already encoded by
    // the Flutter side as URL-safe base64; ResponseFile must use them raw
    // (no second utf8 conversion).
...
std::string HttpServer::map_url_to_file_path(const std::string& url)
{
    ...
    if (trimmed_url == "/") {
        ...
        // Upstream-snap 1c2df8e498: /wcp_download/<base64-path> route
        // bypasses the 512 MB postMessage limit by streaming the file
        // through HTTP. The path component is URL-safe base64 ('+'→'-',
        // '/'→'_'), so revert before decoding and pad to a multiple
        // of 4.
        auto b64 = std::string(trimmed_url.substr(strlen(WCP_DOWNLOAD_PREFIX))
        ...
        auto result = boost::beast::detail::base64::decode(decoded.data(),
                                                           b64.data(),
                                                           b64.size());
...
void HttpServer::ResponseFile::write_response(std::stringstream& ssOut)
{
    ...
    // Upstream-snap 1c2df8e498: m_native_path == true (set by the
    // /wcp_download/ branch) means the path was already base64-decoded
    // and is in native filesystem bytes — skip the utf8 conversion and
    // open as-is. Otherwise try the converted path, then fall back to
    // the raw bytes if that file doesn't exist either.
    std::ifstream file;
    if (m_native_path) {
